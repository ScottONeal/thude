...
A:RegisterEvent("COMBAT_LOG_EVENT")
...
frameWindow:SetMovable(false)
frameWindow:EnableMouse(false)
if tailleicon == 16 then
	frameWindow:SetWidth(150);
	frameWindow:SetHeight(32);
elseif tailleicon == 32 then
...
local function handler(msg, editbox)
	...
	elseif msg == 'help' then
		if lang == "EN" then
			...
			DEFAULT_CHAT_FRAME:AddMessage("     icon32 - icon size 16x16", 1, 1, .5);
			...
			DEFAULT_CHAT_FRAME:AddMessage("     icon32 - Taille icône 16x16", 1, 1, .5);
	...
	elseif msg == 'icon16' then
		...
		if(horizontaly)then
			...
			frameWindow:SetWidth(32);
			frameWindow:SetHeight(150);
			FrameWidth = 32
			FrameHeight = 150
			XRollPosDB = {
				["FrameWidth"] = FrameWidth,
		...
		fra:SetValue(0)
		tailleFontTextattack = 7
		if lang == "EN" then
			DEFAULT_CHAT_FRAME:AddMessage("[XRoll] say : Icon is 16x16!", 1, 1, .5);
		else
			DEFAULT_CHAT_FRAME:AddMessage("[XRoll] dit : Taille icône 16x16!", 1, 1, .5);
		end
...
Z:SetScript("OnEvent",function(self, event, ...)
	local timestamp, type, hideCaster, sourceGUID, sourceName, sourceFlags, sourceFlags2, destGUID, destName, destFlags, destFlags2 = select(1, ...)
	  -- Note, for this example, you could just use 'local type = select(2, ...)'.  The others are included so that it's clear what's available.
		...
		if(sourceName == MyID) then
			if (type=="SPELL_DAMAGE") then
				local spellId, spellName, spellSchool = select(12, ...)
				local amount, overkill, school, resisted, blocked, absorbed, critical, glancing, crushing = select(15, ...)

				if (spellId==57841 and ifsoundisplayingultra == false and soundPlay == true) then -- ==true for clarity only.  Not needed.
			...
			for i=1,40 do local name,_,_,count,_,duration,expires,_,_,_,spellID, timeMod = UnitBuff("player", i)
				if(spellID)then
					...
					if (spellID == 193356 or spellID == 193357 or spellID == 193358 or spellID == 199603 or spellID == 199600 or spellID == 193359) then
						...
						if dudu2 <= 5 then
							if ifsoundend == false and soundPlay == true then
								PlaySoundFile("Interface\\AddOns\\XRoll\\Sound\\end.mp3", "Master")
								ifsoundend = true;
							end
						end
						...
						if e == 1 then
							...
							f6:Hide()
							t1:SetTexture(icone)
							--t1:SetMask("Interface\\ARCHEOLOGY\\Arch-Keystone-Mask")
							f1.texture = t1
							f1.MaxLabel:SetFont("Fonts\\FRIZQT__.TTF", tailleFontTextattack, "OUTLINE")
							
							if tailleicon == 16 then
								f1.MaxLabel:SetSize(65, 60)
							...
							elseif spell1 == 193357 then
								...
								if lang == "EN" then
									f1.MaxLabel:SetText("CRITICAL")
								else
									f1.MaxLabel:SetText("CRITIQUE")
								end
							elseif spell1 == 193358 then
								if lang == "EN" then
									f1.MaxLabel:SetText("ATT SPEED.")
								else
									f1.MaxLabel:SetText("VITESSE ATT.")
								end
							elseif spell1 == 199603 then
								if lang == "EN" then
									f1.MaxLabel:SetText("SABER X2")
								else
									f1.MaxLabel:SetText("SABRE X2")
								end
							elseif spell1 == 199600 then
								f1.MaxLabel:SetText("REGEN")
							elseif spell1 == 193359 then
								if lang == "EN" then
									f1.MaxLabel:SetText("REDUCE CD")
								else
									f1.MaxLabel:SetText("REDUIT CD")
								end
							end
							
							if textAttackTop == true then
								f1.MaxLabel:ClearAllPoints();
								f1.MaxLabel:SetPoint("TOP", 0, 40)
							else
								f1.MaxLabel:ClearAllPoints();
								f1.MaxLabel:SetPoint("BOTTOM", 0, -40)
							end
						...
						elseif e == 2 then
							...
							if tailleicon == 16 then
								f2.MaxLabel:SetSize(65, 60)
								if(horizontaly) then
									f1:SetPoint("CENTER", frameWindow, "CENTER", 25, 0)
									f2:SetPoint("CENTER", frameWindow, "CENTER", -25, 0)
								else
									f1:SetPoint("CENTER", frameWindow, "CENTER", 0, 25)
									f2:SetPoint("CENTER", frameWindow, "CENTER", 0, -25)
								end
							elseif tailleicon == 32 then
							...
							elseif spell2 == 193357 then
								...
								if lang == "EN" then
									f2.MaxLabel:SetText("CRITICAL")
								else
									f2.MaxLabel:SetText("CRITIQUE")
								end
							elseif spell2 == 193358 then
								if lang == "EN" then
									f2.MaxLabel:SetText("ATT SPEED.")
								else
									f2.MaxLabel:SetText("VITESSE ATT.")
								end
							elseif spell2 == 199603 then
								if lang == "EN" then
									f2.MaxLabel:SetText("SABER X2")
								else
									f2.MaxLabel:SetText("SABRE X2")
								end
							elseif spell2 == 199600 then
								f2.MaxLabel:SetText("REGEN")
							elseif spell2 == 193359 then
								if lang == "EN" then
									f2.MaxLabel:SetText("REDUCE CD")
								else
									f2.MaxLabel:SetText("REDUIT CD")
								end
							end
							
							if textAttackTop == true then
								f2.MaxLabel:ClearAllPoints();
								f2.MaxLabel:SetPoint("TOP", 0, 40)
							else
								f2.MaxLabel:ClearAllPoints();
								f2.MaxLabel:SetPoint("BOTTOM", 0, -40)
							end
						...
						elseif e == 3 then
							...
							if tailleicon == 16 then
								f3.MaxLabel:SetSize(65, 60)
								if(horizontaly) then
									f1:SetPoint("CENTER", frameWindow, "CENTER", 50, 0)
									f2:SetPoint("CENTER", frameWindow, "CENTER", 0, 0)
									f3:SetPoint("CENTER", frameWindow, "CENTER", -50, 0)
								else
									f1:SetPoint("CENTER", frameWindow, "CENTER", 0, 50)
									f2:SetPoint("CENTER", frameWindow, "CENTER", 0, 0)
							...
							elseif spell3 == 193357 then
								...
								if lang == "EN" then
									f3.MaxLabel:SetText("CRITICAL")
								else
									f3.MaxLabel:SetText("CRITIQUE")
								end
							elseif spell3 == 193358 then
								if lang == "EN" then
									f3.MaxLabel:SetText("ATT SPEED.")
								else
									f3.MaxLabel:SetText("VITESSE ATT.")
								end
							elseif spell3 == 199603 then
								if lang == "EN" then
									f3.MaxLabel:SetText("SABER X2")
								else
									f3.MaxLabel:SetText("SABRE X2")
								end
							elseif spell3 == 199600 then
								f3.MaxLabel:SetText("REGEN")
							elseif spell3 == 193359 then
								if lang == "EN" then
									f3.MaxLabel:SetText("REDUCE CD")
								else
									f3.MaxLabel:SetText("REDUIT CD")
								end
							end
							
							if textAttackTop == true then
								f3.MaxLabel:ClearAllPoints();
								f3.MaxLabel:SetPoint("TOP", 0, 40)
							else
								f3.MaxLabel:ClearAllPoints();
								f3.MaxLabel:SetPoint("BOTTOM", 0, -40)
							end
						...
						elseif e == 4 then
							...
							if tailleicon == 16 then
								f4.MaxLabel:SetSize(65, 60)
								if(horizontaly) then
									f1:SetPoint("CENTER", frameWindow, "CENTER", 75, 0)
									f2:SetPoint("CENTER", frameWindow, "CENTER", 25, 0)
									f3:SetPoint("CENTER", frameWindow, "CENTER", -25, 0)
									f4:SetPoint("CENTER", frameWindow, "CENTER", -75, 0)
								else
									f1:SetPoint("CENTER", frameWindow, "CENTER", 0, 75)
									f2:SetPoint("CENTER", frameWindow, "CENTER", 0, 25)
									f3:SetPoint("CENTER", frameWindow, "CENTER", 0, -25)
							...
							elseif spell4 == 193357 then
								...
								if lang == "EN" then
									f4.MaxLabel:SetText("CRITICAL")
								else
									f4.MaxLabel:SetText("CRITIQUE")
								end
							elseif spell4 == 193358 then
								if lang == "EN" then
									f4.MaxLabel:SetText("ATT SPEED.")
								else
									f4.MaxLabel:SetText("VITESSE ATT.")
								end
							elseif spell4 == 199603 then
								if lang == "EN" then
									f4.MaxLabel:SetText("SABER X2")
								else
									f4.MaxLabel:SetText("SABRE X2")
								end
							elseif spell4 == 199600 then
								f4.MaxLabel:SetText("REGEN")
							elseif spell4 == 193359 then
								if lang == "EN" then
									f4.MaxLabel:SetText("REDUCE CD")
								else
									f4.MaxLabel:SetText("REDUIT CD")
								end
							end
							
							if textAttackTop == true then
								f4.MaxLabel:ClearAllPoints();
								f4.MaxLabel:SetPoint("TOP", 0, 40)
							else
								f4.MaxLabel:ClearAllPoints();
								f4.MaxLabel:SetPoint("BOTTOM", 0, -40)
							end
						...
						elseif e == 5 then
							...
							if tailleicon == 16 then
								f5.MaxLabel:SetSize(65, 60)
								if(horizontaly) then
									f1:SetPoint("CENTER", frameWindow, "CENTER", 100, 0)
									f2:SetPoint("CENTER", frameWindow, "CENTER", 50, 0)
									f3:SetPoint("CENTER", frameWindow, "CENTER", 0, 0)
									f4:SetPoint("CENTER", frameWindow, "CENTER", -50, 0)
									f5:SetPoint("CENTER", frameWindow, "CENTER", -100, 0)
								else
							...
							elseif spell5 == 193357 then
								...
								if lang == "EN" then
									f5.MaxLabel:SetText("CRITICAL")
								else
									f5.MaxLabel:SetText("CRITIQUE")
								end
							elseif spell5 == 193358 then
								if lang == "EN" then
									f5.MaxLabel:SetText("ATT SPEED.")
								else
									f5.MaxLabel:SetText("VITESSE ATT.")
								end
							elseif spell5 == 199603 then
								if lang == "EN" then
									f5.MaxLabel:SetText("SABER X2")
								else
									f5.MaxLabel:SetText("SABRE X2")
								end
							elseif spell5 == 199600 then
								f5.MaxLabel:SetText("REGEN")
							elseif spell5 == 193359 then
								if lang == "EN" then
									f5.MaxLabel:SetText("REDUCE CD")
								else
									f5.MaxLabel:SetText("REDUIT CD")
								end
							end
							
							if textAttackTop == true then
								f5.MaxLabel:ClearAllPoints();
								f5.MaxLabel:SetPoint("TOP", 0, 40)
							else
								f5.MaxLabel:ClearAllPoints();
								f5.MaxLabel:SetPoint("BOTTOM", 0, -40)
							end
						...
						elseif e == 6 then
							...
							if tailleicon == 16 then
								f6.MaxLabel:SetSize(65, 60)
								if(horizontaly) then
									f1:SetPoint("CENTER", frameWindow, "CENTER", 125, 0)
									f2:SetPoint("CENTER", frameWindow, "CENTER", 75, 0)
									f3:SetPoint("CENTER", frameWindow, "CENTER", 25, 0)
									f4:SetPoint("CENTER", frameWindow, "CENTER", -25, 0)
									f5:SetPoint("CENTER", frameWindow, "CENTER", -75, 0)
									f6:SetPoint("CENTER", frameWindow, "CENTER", -125, 0)
								else
									f1:SetPoint("CENTER", frameWindow, "CENTER", 0, 125)
									f2:SetPoint("CENTER", frameWindow, "CENTER", 0, 75)
									f3:SetPoint("CENTER", frameWindow, "CENTER", 0, 25)
									f4:SetPoint("CENTER", frameWindow, "CENTER", 0, -25)
									f5:SetPoint("CENTER", frameWindow, "CENTER", 0, -75)
									f6:SetPoint("CENTER", frameWindow, "CENTER", 0, -125)
							...
							elseif spell6 == 193357 then
								...
								if lang == "EN" then
									f6.MaxLabel:SetText("CRITICAL")
								else
									f6.MaxLabel:SetText("CRITIQUE")
								end
							elseif spell6 == 193358 then
								if lang == "EN" then
									f6.MaxLabel:SetText("ATT SPEED.")
								else
									f6.MaxLabel:SetText("VITESSE ATT.")
								end
							elseif spell6 == 199603 then
								if lang == "EN" then
									f6.MaxLabel:SetText("SABER X2")
								else
									f6.MaxLabel:SetText("SABRE X2")
								end
							elseif spell6 == 199600 then
								f6.MaxLabel:SetText("REGEN")
							elseif spell6 == 193359 then
								if lang == "EN" then
									f6.MaxLabel:SetText("REDUCE CD")
								else
									f6.MaxLabel:SetText("REDUIT CD")
								end
							end
							
							if textAttackTop == true then
								f6.MaxLabel:ClearAllPoints();
								f6.MaxLabel:SetPoint("TOP", 0, 40)
							else
								f6.MaxLabel:ClearAllPoints();
								f6.MaxLabel:SetPoint("BOTTOM", 0, -40)
							end
...
local buuffs = {
	["Bordées"] = true,     	 --French
	["Broadsides"] = true, 	  	 --English
	["Breitseiten"] = true, 	 --Deutsh
	["Andanadas"] = true,   	 --spania
	["Bordate"] = true,     	 --italiano
	["Canhões de Bordo"] = true, --Portugûes
	["Бортовые залпы"] = true,	 --Russe
	["집중 공격"] = true,			 
	["强势连击"] = true,
	
	["Eaux infestées de requins"] = true,
	["Shark Infested Waters"] = true,
	["Haiverseuchte Gewässer"] = true,
	["Aguas con tiburones"] = true,
	["Acque Infestate di Squali"] = true,
	["Água Infestada de Tubarões"] = true,
	["Море акул"] = true,
	["상어가 가득한 바다"] = true,
	["暗鲨涌动"] = true,
	...
	["Gran refriega"] = true,
	["Granmischia"] = true,
	["Corpo a Corpo Proficiente"] = true,
	["Великая битва"] = true,
	["대난투"] = true,
	["大乱斗"] = true,
	...
	["True Bearing"] = true,
	["Kurs halten"] = true,
	["Rumbo verdadero"] = true,
	["Puntata Alta"] = true,
	["Postura Perfeita"] = true,
	["Истинный азимут"] = true,
	["진방위"] = true,
	["精准定位"] = true,
	
	["Trésor enfoui"] = true,
	["Buried Treasure"] = true,
	["Vergrabener Schatz"] = true,
	["Tesoro enterrado"] = true,
	["Tesoro Sepolto"] = true,
	["Tesouro Enterrado"] = true,
	["Зарытое сокровище"] = true,
	["숨겨진 보물"] = true,
	["埋藏的宝藏"] = true,
	
	["Pavillon noir"] = true,
	["Jolly Roger"] = true,
	["Gezinkte Würfel"] = true,
	["Bandera negra"] = true,
	["Bandiera Pirata"] = true,
	["Bandeira Negra"] = true,
	["Веселый Роджер"] = true,
	["해적기"] = true,
	["骷髅黑帆"] = true,
...
IconImageText:SetText("XRoll v" .. VERSION .. " - Create By " .. AUTHOR .. "-Archimonde EU")
...
fra:SetScript("OnValueChanged", function(self, value)
	...
	elseif value >= 33 and value < 66 then
		...
		f7:SetHeight(tailleicon)
		tailleFontTextattack = 8
	elseif value >= 66 and value < 100 then
		tailleicon = 48
		if(horizontaly)then
			frameWindow:SetWidth(350);
			frameWindow:SetHeight(72);
...
fra.MinLabel:SetText("16x16")

fra.MinLabel2 = fra:CreateFontString(nil, "Overlay")
fra.MinLabel2:SetFontObject(GameFontHighlightSmall)
fra.MinLabel2:SetSize(0, 14)
fra.MinLabel2:SetWordWrap(false)
fra.MinLabel2:SetPoint("TopLeft", fra, "BottomLeft", 55, -1)
local min, max = fra:GetMinMaxValues()
fra.MinLabel2:SetText("32x32")
...
fraCenterLabel:SetPoint("TopRight", fra, "BOTTOM", 45, -1)
...
XRoll_eventFrame2:SetScript("OnEvent", function(self,event,...) 
	if type(XRollPosDB) ~= "table" then
		...
		if XRollPosDB["VERSION"] == VERSION then
			...
			soundPlay = XRollPosDB["soundPlay"];
			horizontaly = XRollPosDB["horizontaly"];
			XRollPosDB["VERSION"] = VERSION
			if lang == "EN" then
				DEFAULT_CHAT_FRAME:AddMessage("[XRoll] say : Update to version " .. VERSION .. " applied.", 1, 1, .5);
			else
				DEFAULT_CHAT_FRAME:AddMessage("[XRoll] dit : Mise à jour vers la version " .. VERSION .. " appliquée.", 1, 1, .5);
			end
			ADDON_APPLY();
		end
	end
...
function XRoll_eventFrame:PLAYER_LOGIN()
	DEFAULT_CHAT_FRAME:AddMessage("XRoll v" .. VERSION .. " loaded:  /xr help", 1, 1, .5);
	DEFAULT_CHAT_FRAME:AddMessage("Create by " .. AUTHOR .. "-Archimonde EU", 1, 1, .5);
end
